...
void MAG3110::initialize(const char* device_name) {
	ic2Fd = wiringPiI2CSetupInterface(device_name, MAG3110_I2C_ADDRESS);
...
uint8_t MAG3110::readRegister(uint8_t address){
  uint8_t output=0;
  // wiringPiI2CWrite(ic2Fd, address);
  ...
  output = wiringPiI2CReadReg8(ic2Fd, address);
...
void MAG3110::writeRegister(uint8_t address, uint8_t value){
  ...
  // Wire.write(value);
  // Wire.endTransmission();
	wiringPiI2CWriteReg8(ic2Fd, address, value);
}
...
void MAG3110::setAutoRest(bool activate)
{	
	uint8_t current = readRegister(MAG3110_CTRL_REG2);
	if (activate) //Turn on Auto reset mode
	{
		autoReset = true;
		writeRegister(MAG3110_CTRL_REG2, current | MAG3110_AUTO_MRST_EN );
	}
	else //Turn off auto reset
	{
		autoReset = false;
		writeRegister(MAG3110_CTRL_REG2, current & (~MAG3110_AUTO_MRST_EN));
	}
...
bool MAG3110::isActive() const
{
	return activeMode;
}

bool MAG3110::isRaw() const
{
	return rawMode;
}

bool MAG3110::isCalibrated() const
{
	return calibrated;
}

bool MAG3110::isCalibrating() const
{
...
void MAG3110::calibration_loop()
{
	for(;;)
	{
		if (!isCalibrated()) //If we're not calibrated
		{
			if (!isCalibrating()) //And we're not currently calibrating
			{
...
void MAG3110::exitCalMode(){
	//Calculate offsets
	x_offset = static_cast<int>(x_min + x_max)/2;

	y_offset = static_cast<int>(y_min + y_max)/2;
	
	z_offset = static_cast<int>(z_min + z_max) / 2;
	...
	setOffset(MAG3110_Z_AXIS, z_offset);
